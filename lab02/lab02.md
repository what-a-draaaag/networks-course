...
Если не все поля указаны:
![alt text](image-11.png)

2. Получить продукт по его id

![alt text](image-2.png)
Если запросить id, которого нет:
...
После добавления еще нескольких продуктов:
...
Запросили картинку, которую пока не отправляли
![alt text](image-13.png)
![alt text](image-14.png)
Теперь картинка есть
...
Первый пакет проходит все $N$ участков за $N \cdot \frac{L}{R}$.
Остальные $P-1$ пакетов идут следом, каждый добавляет только время одной передачи $\frac{L}{R}$, так как не ждет освобождения всего пути. Все сложим и получим $d_{сквозная}$ = $N \cdot \frac{L}{R}$ + ${(P-1)} \cdot \frac{L}{R}$
