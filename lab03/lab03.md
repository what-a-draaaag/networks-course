...
#### Решение
Размер пакета $56 байт = 448 бит$, время сборки пакета $\frac{448}{128} = 3,5 мс$. Посчитаем время передачи пакета, прибавим к нему время сборки и задержку распространения: $\frac{448}{1000} мс + 3,5мс + 5мс = 8,948 мс$.
...
#### Решение
$N = 11$. Скорость линии $100 пак./с$, значит, время передачи одного пакета $1/100 = 10 мс$. Задержка ожидания равна $10мс$, полная задержка = $10 мс + 10 мс = 20 мс = 0,02 с$. Подставим в формулу Литтла $N = a \cdot d$, $11 = a \cdot 0,02$. $a = \frac{11}{0,02} = 550$
...
#### Решение
1. Так как узкое место это первая линия, пакеты идут без накопления очереди на маршрутизаторе и разница прихода последних битов равна интервалу отправки сервером: $L / R_s$

2. Да, может, первая линия быстрее второй, так что второй пакет может прийти на маршрутизатор раньше, чем освободится вторая линия после передачи первого пакета. Чтобы очереди не было, второй пакет должен прийти не раньше, чем вторая линия освободится:

$T + \frac{L}{R_s} + d_{распространения} ≥ \frac{L}{R_s} + d_{распространения} + \frac{L}{R_c}$
