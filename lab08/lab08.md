...
#### Решение
Увеличение окна происходит линейно от $\frac{W}{2}$ до $W$, поэтому среднее значение $\frac{3W}{4}$, чтобы найти среднюю пропускную способность поделим на $RTT$: $A=\frac{3W}{4 \cdot RTT}$. Выразим отсюда $W = \frac{A \cdot 4 \cdot RTT}{3}$. Время роста окна в RTT составляет $\frac{W}{2}$, подставим $W$:

$T=\frac{W}{2} = \frac{A \cdot 4 \cdot RTT}{3 \cdot 2} =\frac{A \cdot 2 \cdot RTT}{3}$
...
1. В медленном старте TCP окно растет так: 1 сегмент, 2,4,8...,как раз хватает: `1 + 2 + 4 + 8 = 15`, обозначим время передачи одного сегмента `x = S / R`. Перед самой передачей данных учитываем 2 · RTT, один RTT на установление TCP-соединения, еще один на запрос объекта и получение возможности начать передачу данных. Сами данные передаем за `15 · S / R`

По условию пункта 1: после отправки 2 сегментов сервер еще немного ждет ACK, но после отправки 4 сегментов уже не простаивает

Простой после первой группы:`RTT`, простой после второй группы: `RTT - S/R`, после группы из `4` сегментов простоя уже нет. Получаем итоговое время `T = 2RTT + 15S/R + RTT + (RTT - S/R) = 4RTT + 14S/R`

2.  Сервер простаивает после групп из 1, 2 и 4 сегменов: первый простой `RTT`, второй `RTT - S/R`, третий `RTT - 3S/R`. Итоговое время: `T = 2RTT + 15S/R + RTT + (RTT - S/R) + (RTT - 3S/R)= 5RTT + 11S/R`

3. Здесь сегмент передается дольше, чем RTT, поэтому после первой паузы дальше сервер уже не ждет ACK, есть только простой после первой группы: `T = 2RTT + 15S/R + RTT =  3RTT + 15S/R`
